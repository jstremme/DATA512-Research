...
- Apply additional models to evaluate classification accuracy with these additional features to compare to phase 1.
- Generate relevant plots to explain predictor variables and their relationship to the target variable.

My reason for using a model to answer my questions in addition to examining mean failures by food establishment and geographical category types is to control for the other features present in the data to understand, in the presence of all available features, which categories and geographical regions are most predictive of failures.
